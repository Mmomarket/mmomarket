...
export default function CarteiraPage() {
  ...
  const loadWithdrawals = useCallback(() => {
    ...
    fetch("/api/withdrawals")
      .then((r) => r.json())
      .then((data) => setWithdrawals(data.withdrawals ?? data))
  ...
  const handleDeposit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/deposits", {
        ...
        body: JSON.stringify({ amountBRL: parseFloat(depositAmount) }),
      });
      const data = await res.json();
      if (!res.ok) {
        setError(data.error || "Erro ao criar deposito");
        return;
      }
      if (data.paymentUrl) {
        setSuccess("Deposito criado! Redirecionando...");
        setTimeout(() => {
          window.open(data.paymentUrl, "_blank");
        }, 1000);
      } else {
        setSuccess("Deposito criado com sucesso!");
      }
      setDepositAmount("");
      loadDeposits();
    } catch {
      setError("Erro ao criar deposito");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const depositForm = (
    <form onSubmit={handleDeposit} className="space-y-4">
      ...
      <Input
        ...
        min="1"
        step="0.01"
        hint="Minimo R$ 1,00. Pagamento via MercadoPago."
      />
      ...
      <Button type="submit" loading={submitting} className="w-full" size="lg">
        Depositar via MercadoPago
      </Button>
      <p className="text-xs text-gray-600 text-center">
        Processado pelo MercadoPago. O saldo e creditado automaticamente.
      </p>
    </form>
  );
  ...
  const withdrawForm = (
    <form onSubmit={handleWithdraw} className="space-y-4">
      ...
      <p className="text-xs text-gray-600 text-center">
        O saque sera processado em ate 24h via Pix.
      </p>
    </form>
  );
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      <div>
        ...
        <p className="text-gray-500 mt-1">
          Gerencie seu saldo, faca depositos e saques em BRL
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <>
                ...
                {wallet.escrowBRL > 0 && (
                  <div>
                    ...
                    <p className="text-lg font-semibold text-orange-400 mt-0.5">
                      {formatBRL(wallet.escrowBRL)}
                    </p>
                    <p className="text-xs text-gray-600 mt-1">
                      Aguardando confirmacao de entrega de moeda no jogo
                    </p>
                  </div>
                )}
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-sm font-medium text-white">
                    {formatBRL(
                      wallet.balanceBRL + wallet.frozenBRL + wallet.escrowBRL,
                    )}
                  </span>
                </div>
              </>
        ...
        <Card>
          <CardContent className="pt-4">
            <Tabs
              tabs={[
                { id: "deposit", label: "Depositar", content: depositForm },
                { id: "withdraw", label: "Sacar", content: withdrawForm },
              ]}
              defaultTab="deposit"
            />
          </CardContent>
            ...
            tabs={[
              {
                ...
                content: loadingDeposits ? (
                  ...
                  <EmptyState
                    title="Nenhum deposito"
                    description="Faca seu primeiro deposito para comecar a negociar!"
                  />
                ) : (
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {deposits.map((deposit) => {
                          const st = depositStatusMap[deposit.status] || {
              ...
              },
              {
                ...
                content: loadingWithdrawals ? (
                  ...
                  <EmptyState
                    title="Nenhum saque"
                    description="Voce ainda nao fez nenhuma solicitacao de saque."
                  />
                ) : (
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {withdrawals.map((w) => {
                          ...
                          const pixDisplay = w.pixKey
                            ? w.pixKey.includes(":")
                              ? w.pixKey.split(":").slice(1).join(":")
                              : w.pixKey
                            : "\u2014";
                          return (
                            <tr
                            ...
                            >
                              ...
                              <td className="py-3 text-right text-gray-400 text-xs font-mono max-w-[120px] truncate">
                                {pixDisplay}
                              </td>
                              <td className="py-3 text-right">
                                <Badge variant={st.variant}>{st.label}</Badge>
